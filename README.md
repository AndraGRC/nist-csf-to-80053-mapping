...
## Added files
- `nist-csf-functions.csv` — CSF starter (Functions/Categories/Subcategories subset).
- `nist-csf-to-80053-mapping.csv` — Example crosswalk entries.
- `coverage_report.py` — Generates coverage charts and summary CSVs.
- `coverage_by_family.png`, `mappings_by_csf_function.png` — Charts generated by the script.
- `summary_family_counts.csv`, `summary_function_counts.csv` — Count summaries.

### How to run the coverage report locally
```bash
python3 coverage_report.py
```
This outputs two charts and two CSV summaries in the repo root.
